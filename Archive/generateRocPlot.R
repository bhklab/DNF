...
#   auc4 <- unlist(slot(auc4, "y.values"))
  ...
  aucLegIntegr <- paste(c("Integration = "), round(predIntegr$auc,2), sep="")
  aucLegStr <- paste(c("Structure = "), round(predStrc$auc,2),sep="")
  aucLegSen <- paste(c("Sensitivity = "), round(predSens$auc,2) , sep="")
  aucLegPer <- paste(c("Perturbation = "), round(predPert$auc,2), sep="")
  
  legend(0.6,0.3,c(aucLegIntegr, aucLegStr, aucLegSen, aucLegPer), border="white", cex=0.75, 
         box.col = "white",fill=c("black","#d7191c","#41ab5d","#2b83ba"),title = "AUC")
  dev.off()
