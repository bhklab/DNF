...
generateRocPlotCombined <- function(allPairs, d1Name, d2Name="lincs", benchName) {
    ...
    plot(predSens$perf, col = "#41ab5d", lwd=2)
    ...
    if (length(allPairs)==9 & !is.null(allPairs$superPairs)) {
        ...
        legend(0.5,0.4,c(aucLegSen, aucLegIorio, aucLegIskar, aucLegSuper,rand), border="white", cex=0.75,
               box.col = "white",fill=c("black","#d7191c","#41ab5d","#2b83ba", "pink"))
    }
    if (length(allPairs)==9 & !is.null(allPairs$drugePairs)) {
        aucLegDrugE <- paste(c("DrugERank = "), round(predDrugE$auc,3), sep="")
        legend(0.5,0.4,c(aucLegSen, aucLegIorio, aucLegIskar, aucLegDrugE,rand), border="white", cex=0.75,
               box.col = "white",fill=c("black","#d7191c","#41ab5d","#2b83ba", "pink"))
    }
    
    legend(0.5,0.4,c(aucLegSen, aucLegIorio, aucLegIskar, rand), border="white", cex=0.75,
           box.col = "white",fill=c("black","#d7191c","#2b83ba", "pink"))
