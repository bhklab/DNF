# This script determines a list of drugs from L1000 intersected with Cell Painting
# that also have drug target information available.
...
l1000.inter.painting <- read.csv("Data/l1000_cellpainting.csv", stringsAsFactors = FALSE)
cdrugs <- l1000.inter.painting$name
...
drugs.with.target.info <- unique(Reduce(union, 
                                        list(chembl.drug.targs$MOLECULE_NAME, 
                                             clue.targets$MOLECULE_NAME, 
                                             ctrp.drug.targs$MOLECULE_NAME,
                                             dtc.targs$MOLECULE_NAME,
                                             uniprot.targs$MOLECULE_NAME)))

which(cdrugs %in% drugs.with.target.info)

indices <- match(drugs.with.target.info, cdrugs)

for.chantal <- l1000.inter.painting[indices, ]

saveRDS(for.chantal, "l1000_painting_drugs_that_have_targets.RData")
