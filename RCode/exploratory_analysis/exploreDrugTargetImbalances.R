# This script creates a barplot showing the distribution of
# drug targets. The goal is to investigate if just a handful of targets 
# are responsible for the AUC results I.e.if a single target shows
# up 100 times, such an imbalance might be of interest.
...
cdrugs <- intersect(rownames(sensitivity.data), lincs.meta$pert_iname)
...
targets <- c(clue.targets$TARGET_NAME, ctrp.drug.targs$TARGET_NAME,
                uniprot.targs$TARGET_NAME, dtc.targs$TARGET_NAME)

target.counts <- table(targets)
barplot(sort(target.counts, decreasing=TRUE)[1:100])
sum(sort(target.counts, decreasing=TRUE)[1:20])
