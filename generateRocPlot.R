...
#   auc4 <- unlist(slot(auc4, "y.values"))
  ...
  plot(predStrc$perf, col="red", add = TRUE)
  plot(predSens$perf, col = "green", add = TRUE)
  plot(predPert$perf, col = "blue", add = TRUE)
  
  aucLegIntegr <- paste(c("Combiall AUC  = "), predIntegr$auc, sep="")
  aucLegStr <- paste(c("Str AUC = "), predStrc$auc,sep="")
  aucLegSen <- paste(c("Sens AUC = "), predSens$auc , sep="")
  aucLegPer <- paste(c("Pert AUC = "), predPert$auc, sep="")
  
  legend(0.6,0.3,c(aucLegIntegr, aucLegStr, aucLegSen, aucLegPer, "\n"), border="white", cex=0.75, box.col = "white")
  dev.off()
