...
for (i in imputation.sizes) {
    ...
    temp <- pert.data[, common.drugs]
    temp <- temp[, !(colnames(temp) %in% drugs.to.remove)]
    subsetted.pert.cor[[as.character(i)]] <- cor(temp, method="pearson", use="pairwise.complete.obs")
    
    print(dim(subsetted.pert.cor[[as.character(i)]]))
}
```
...
```{r}
all.drugs <- sort(Reduce(union, list(rownames(strc.cor), rownames(sens.cor))))
```
...
Finally, we loop over the various subsetted versions of the pert layer and compute performance
...
## Using New Perturbation Signatures

### Performance on CTRPv2

We begin by loading in the new perturbation signatures
...
Let's get the drugs that are found in the ctrpv2 drug target dataset
...
Now we loop over the subsetted datasets and evaluate performance
