...
Instead of considering the top 5 predicted targets, we first determine the top 5 score votes. For a given drug, it might be something like 0.5, 0.4, 0.3, 0.2, 0.1 and then find all the targets with those vote scores. This means that we could be predicting many more targets, but this approach is more principled in that we aren' throwing away target simply because of ordering.
...
top.predictions <- sapply(1:nrow(counts), function(i, target.names, drugs.with.targets) {
    x <- counts[i, ]
    top.5.counts <- x[!duplicated(x)]
    top.5.counts <- top.5.counts[top.5.counts > 0]
    temp <- sort(top.5.counts, index.return=T, decreasing=T)
    top.5.counts <- head(temp$x, top)
    
    indices <- which(x %in% top.5.counts)
    
    
    res <- list()
    res[[drugs.with.targets[i]]] <- x[indices]
    names(res[[drugs.with.targets[i]]]) <- target.names[indices]
    res
...
old.acc <- sum(old.prediction.status) / length(old.prediction.status)

cat("New Drugs Correctly Predicted\n")
print(names(new.prediction.status[new.prediction.status != old.prediction.status]))

cat(paste("Old accuracy: ", old.acc))
cat("\n")
cat(paste("New accuracy: ", new.acc))
cat("\n")

temp <- lapply(top.predictions, length)
average.num.targets <- mean(unlist(temp))

cat(paste("Average number of targets: ", average.num.targets, sep=""))
