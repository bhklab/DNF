...
for (combo in names(integrations)) {
    temp <- GetKNNAccuracy(k, integrations[[combo]], data.bench)
    
    temp <- temp[temp == 0]
    
    mistakes[[combo]] <- names(temp)
}
```
...
For each drug that was not correctly predicted according to our top 5 accuracy metric, we will add that drug's true targets to a count vector

```{r, eval=F}
bad.targets <- numeric(length(target.names))
names(bad.targets) <- target.names

for (i in bad.performers) {
    true.targets <- data.bench$TARGET_NAME[data.bench$MOLECULE_NAME == i]
    
    bad.targets[true.targets] <- bad.targets[true.targets] + 1
}

bad.targets <- bad.targets[bad.targets != 0]
```
