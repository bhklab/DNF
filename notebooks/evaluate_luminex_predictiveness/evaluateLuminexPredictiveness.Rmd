---
title: "Investigate Predictiveness of Luminex Layer"
output:
  html_document: default
  html_notebook: default
---

Now that we have our imputation method, we can see if the luminex layer helps AUC, without looking at a benchmark that is reduced in terms of drugs.
...
Load in senstivity, luminex data as well as perturbation signatures
...
luminex.file.name <- "Data/luminex_processed/luminex_50_pca.rds"
...
luminex.data <- LuminexDataFlexible(badchars, luminex.file.name)

luminex.data <- luminex.data[, colnames(luminex.data) %in% colnames(sens.data)]
```
...
luminex.cor <- ConstLuminexLayerFlexible(luminex.data)
pert.cor <- cor(pert.data[, intersect(common.drugs, colnames(pert.data))], method="pearson", use="pairwise.complete.obs")

registerDoParallel(4)
# Impute missing data
pert.cor <- missForest(pert.cor, maxiter=10, parallelize = "variables")$ximp
...
correlation.matrices <- list(sens=sens.cor, pert=pert.cor, strc=strc.cor, luminex=luminex.cor)
...
## Evaluate Performance With luminex
...
CompDrugTargetBenchmarkFlexible(common.drugs=common.drugs, gmt.file.name="temp",
                                ...
                                integration=integrated, luminex.aff.mat=affinity.matrices$luminex,
...
## Evaluate Performance Without luminex

First, we have to recreate the network with the luminex layer removed
...
As can be seen in the above plots, there is no significant improvement in AUC when we include the luminex layer. This suggests that the added complexity of having it as another layer is not worth the slight performance boost gain, which very well could just be noise