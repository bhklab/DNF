...
source("notebooks/evaluate_imputation_performance/helpers/ImputationPerformanceHelpers.R")
...
for (i in imputation.sizes) {
    ...
    temp.pert <- pert.cor[common.drugs, common.drugs]
    subsetted.pert.cor[[as.character(i)]] <- temp.pert[!(rownames(temp.pert) %in% drugs.to.remove),
                                                       !(colnames(temp.pert) %in% drugs.to.remove)]
        
    temp.sens <- sens.cor[common.drugs, common.drugs]
    subsetted.sens.cor[[as.character(i)]] <- temp.sens[!(rownames(temp.sens) %in% drugs.to.remove), 
                                                       !(colnames(temp.sens) %in% drugs.to.remove)]
    
    temp.strc <- strc.cor[common.drugs, common.drugs]
    subsetted.strc.cor[[as.character(i)]] <- temp.strc[!(rownames(temp.strc) %in% drugs.to.remove),
                                    !(colnames(temp.strc) %in% drugs.to.remove)]
}
```
...
for (i in imputation.sizes) {
    pert.temp.cor <- subsetted.pert.cor[[as.character(i)]]
    sens.temp.cor <- subsetted.sens.cor[["0"]]
    strc.temp.cor <- subsetted.strc.cor[["0"]]
    
    temp <- IntegrateImputedLayers(sens.temp.cor, pert.temp.cor, strc.temp.cor, all.drugs)
    integrated <- temp$integrated
    affinity.matrices <- temp$affinity.matrices
...
```{r, fig.width=5, fig.height=5}
for (i in imputation.sizes) {
    pert.temp.cor <- subsetted.pert.cor[["0"]]
    sens.temp.cor <- subsetted.sens.cor[[as.character(i)]]
    strc.temp.cor <- subsetted.strc.cor[["0"]]
...
for (i in imputation.sizes) {
    pert.temp.cor <- subsetted.pert.cor[["0"]]
    sens.temp.cor <- subsetted.sens.cor[["0"]]
    strc.temp.cor <- subsetted.strc.cor[[as.character(i)]]
...
### Performance on CTRPv2, CHEMBL, clue.io, and Drugbank combined
